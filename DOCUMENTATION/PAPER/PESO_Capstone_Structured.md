...
This study addresses three connected problems.

First, PESO CSJDM has no web-based decision support system to assist staff in matching applicants to available job openings. Staff rely entirely on personal judgment to cross-reference applicant profiles against the vacancy list for every referral decision, with no dashboard to consolidate workforce data for reporting.

Second, building such a system requires selecting a machine learning algorithm, yet no existing study has identified which among Logistic Regression, Random Forest, and Naïve Bayes performs best on small, imbalanced, government-collected placement data of the type PESO offices hold, making the comparison a necessary step before any reliable system can be deployed.

Third, once the system is built and the best algorithm deployed, it remains unknown whether it meets established software quality standards and whether PESO staff are willing to adopt it, requiring evaluation under both ISO/IEC 25010 and the Technology Acceptance Model.
...
**PESO CSJDM Staff.** As the primary users and host institution, PESO CSJDM and its staff benefit most directly. The system reduces the time and effort required to match applicants to available vacancies by providing a ranked list of suitable job openings with suitability scores, giving staff a structured and quantified basis for every referral decision rather than relying on recall alone. At the institutional level, the Analytical Dashboard gives the office a data-driven picture of its registrant pool and local labor market through two panels: a job seeker panel covering registrant counts, sex distribution, youth, senior citizen, and PWD tallies, educational attainment distribution, employment status, and barangay breakdown by district; and a vacancy panel covering total vacancies and employers by local and overseas, top ten vacancies, top industries, and top placement jobs, supporting the preparation of the office's quarterly employment reports and program planning.
...
**Analytical Dashboard.** The system module that presents registered job seeker and vacancy data through two dashboard panels. The job seeker panel covers total registrants with male and female breakdown, counts of youth, senior citizen, and person with disability registrants, distribution by educational attainment across ten levels, distribution by employment status, and barangay-level distribution separated by District 1 and District 2. The vacancy panel covers total vacancies and employers disaggregated by local and overseas, top ten vacancies, top industries, and top placement jobs. The dashboard supports the preparation of the office's quarterly employment reports.
...
**Logistic Regression.** A supervised classification algorithm that models the probability of each possible label using a linear combination of input feature weights, using a one-vs-rest strategy for multi-class problems. One of the three algorithms compared in this study, selected based on its consistent emergence as a top performer in supervised text classification studies for job recommendation.
...
**Naïve Bayes.** A probabilistic classification algorithm that calculates the likelihood of each possible label given a set of input attributes, assuming each attribute contributes independently. One of the three algorithms compared in this study, selected based on its consistent emergence as a top performer in supervised text classification studies for job and career recommendation.
...
**Public Employment Service Office (PESO).** A government office established under Republic Act No. 8759, mandated to provide free employment services including job referrals, labor market information, and employment program access. In this study, PESO refers specifically to the office of the City of San Jose del Monte, Bulacan.

**Random Forest.** An ensemble classification algorithm that trains multiple decision trees on random subsets of features and records, and aggregates their votes to produce a final label and probability estimate. One of the three algorithms compared in this study, selected based on its consistent emergence as a top performer in supervised text classification studies for job and career recommendation.
...
| **Proposed System** | **Applicant to active vacancy referral for PESO staff** | **TF-IDF + supervised ML classification** | **LR, RF, NB** | **Logistic Regression (Macro F1 0.7370)** | **PESO CSJDM historical placement records** | **Flask web application** | **PESO CSJDM staff only** |
...
Three recurring patterns are evident across the six systems. First, all systems that compare classification algorithms — Tiwari & Upadhyay (2024), Betrand et al. (2025), and Darma et al. (2026) — reach different algorithm rankings depending on the data domain, a divergence reinforced in the literature review where Heakl et al. (2024) found Naïve Bayes outperforming Logistic Regression and Random Forest on TF-IDF features, reversing the ordering Tiwari & Upadhyay (2024) reported on a different dataset, confirming that no single classifier universally outperforms the others and that an independent empirical comparison on the actual target dataset is a necessary precondition for a reliable recommendation engine, not an optional step. Second, systems that depend on external infrastructure beyond a basic web server — Srihari et al. (2025) with its RAG and LLM pipeline, and Heakl et al. (2024) with its GPU-intensive transformer fine-tuning — achieve strong benchmark results but impose hardware and operational requirements that a single municipal employment office cannot sustain. Third, systems that rely on cosine similarity for vacancy matching — Tiwari & Upadhyay (2024) at inference, and Najjar et al. (2021) throughout — presuppose that both the applicant profile and the vacancy record contain rich, comparable descriptive text, a condition that does not hold in PESO CSJDM vacancy records, which provide only employer name and job title.
...
| Dataset Profiling | Profile the historical placement records to examine structure, quality, and completeness; determine which fields are suitable as input features for the recommendation model. | PESO CSJDM historical placement records (CSV/Excel) |
...
| FR-03 | Applicant Management | The system shall allow PESO staff to register and maintain applicant records through two methods: (1) batch upload of a CSV or Excel file exported from PEIS, mapping PEIS field headers to system fields; or (2) direct manual entry through the applicant registration form, also used for walk-in applicants requiring an immediate recommendation. Records include the four ML-required fields (skills, education level, work experience, preferred position) plus supplementary operational fields (barangay, civil status, sex, 4Ps and PWD status, contact information). |
...
| FR-06 | Analytical Dashboard | The system shall provide a dedicated Analytical Dashboard tab presenting job seeker and vacancy data across two panels. The **Job Seeker panel** displays: total registered job seekers with male and female percentage breakdown; counts of youth, senior citizen, and person with disability registrants; distribution by educational attainment across ten levels (Elementary Level, Elementary Graduate, High School Level, High School Graduate, Senior High School Level, Senior High School Graduate, College Level, College Graduate, Vocational, and ALS); distribution by employment status (Unemployed and Employed); and barangay-level distribution separated by District 1 and District 2. The **Vacancy panel** displays: total vacancies with local and overseas breakdown; total employers with local and overseas breakdown; top ten vacancies; top industries; and top placement jobs. The dashboard is designed to support the preparation of the office's quarterly employment reports. |
...
| **Format** | CSV or Excel (.xlsx) exported from the Philippine Employment Information System (PEIS) |
...
**Output.** The framework produces two outputs for PESO CSJDM staff: Ranked Job Vacancies and the Analytical Dashboard.

**Feedback.** A feedback arrow connects the output back to the input, indicating that evaluation results, system usage, and newly accumulated placement data can be fed back into the input layer to support continuous improvement and periodic retraining of the machine learning model.
...
**Rapid Application Development (RAD).** RAD is an iterative software development methodology centered on continuous user involvement through prototyping and feedback (Martin, 1991). RAD was selected over Waterfall and Agile because the system requires close, repeated consultation with PESO CSJDM staff: requirements are validated and refined after each prototype review before the next iteration is built, which fits an office whose workflows are best captured by showing working screens to the staff who use them, and because the fixed capstone timeframe favors RAD's compressed, prototype-driven cycles over Waterfall's sequential phase gates.

**CRISP-DM.** For the analytics component, CRISP-DM guided the development of the recommendation model. CRISP-DM is the standard, industry-independent process model for data mining and machine learning projects, consisting of six iterative phases: Business Understanding → Data Understanding → Data Preparation → Modeling → Evaluation → Deployment (Chapman et al., 2000; Schröer et al., 2021). These six phases map directly to how the Logistic Regression, Random Forest, and Naïve Bayes models were prepared, trained, evaluated, and deployed through the Flask application. The deployed model serves the Job Recommendation Module at inference time without requiring a separate training run.
...
|---|---|---|---|
| 1. Requirements Planning | Conduct interviews and on-site observation with PESO CSJDM staff; profile the historical placement dataset; define functional and non-functional requirements | Business Understanding: identify the referral decision problem, confirm data availability, define model performance goals | Requirements specification; Data Sharing Agreement |
| 2. User Design | Design system architecture, DFDs, ERD, and high-fidelity mockups for all system modules; review prototypes with PESO CSJDM staff; refine per feedback | Data Understanding: examine dataset structure, field completeness, class distribution. Data Preparation: remove incomplete records, fill blank WORK EXPERIENCE entries with 'NO EXPERIENCE', consolidate sparsely represented job titles into occupational categories as the target label, split 80/20 stratified | Validated system design; cleaned dataset; selected feature set |
| 3. Construction | Develop all system modules; integrate the ML pipeline with the Flask application for recommendation inference | Modeling: fit TF-IDF vectorizer on training partition only; train Logistic Regression, Random Forest, and Naïve Bayes on the resulting training vectors. Evaluation: evaluate all three algorithms on the held-out test set using Accuracy, Precision, Recall, and Macro/Weighted F1; select the model with the highest Macro F1. Deployment: serialize the best-performing pipeline with joblib; deploy to the Flask application for live recommendation requests | Working system; algorithm comparison results; deployed recommendation model |
| 4. Cutover | Conduct ISO 25010 and TAM evaluations; apply corrections; prepare handover | — | Evaluated system; handover package |
...
Figure 4 decomposes the system into six processes and five data stores, all accessed by the single external entity, PESO Staff. Process 1, Authenticate User, checks login credentials against the Users store (D1) and records every attempt in the Login Logs store (D4). Process 2, Manage User Accounts, maintains staff accounts in D1 and lets staff review login history from D4. Process 3, Manage Applicant Records, saves manually registered and PEIS-uploaded profiles to the Applicants store (D2). Process 4, Manage Job Vacancies, maintains vacancy records in the Job Vacancies store (D3). Process 5, Generate Job Recommendations, reads an applicant profile from D2 and the active vacancy list from D3, returns a ranked recommendation list to staff, and logs it to the Recommendations store (D5). Process 6, Display Analytics Dashboard, reads the Applicants, Job Vacancies, and Recommendations stores to produce the dashboard report.
...
Figure 5 presents the use case diagram. The single actor, PESO Staff, interacts with twelve primary use cases, several of which are decomposed into finer sub-use-cases through UML «include» relationships, which denote mandatory sub-steps always performed as part of the base use case, and «extend» relationships, which denote optional or conditional actions.

Under **access and account management**, the primary use cases are Log In, Log Out, Manage User Accounts, and Change Password. Log In includes Record Login Attempt, since every attempt is written to the login log, and is extended by Reject Invalid Login when credentials fail. Manage User Accounts is extended by Create Staff Account, Edit Staff Account, Deactivate / Reactivate Account, and Review Login History, the last being the security audit of authentication events.

Under **applicant management**, the primary use cases are Register Applicant, Upload Applicant Batch, and Manage Applicant Records. Register Applicant and Upload Applicant Batch both include Validate Required Fields, which enforces the four machine-learning fields; Upload Applicant Batch additionally includes Map PEIS Fields and is extended by Flag Invalid Rows. Manage Applicant Records is extended by Edit Applicant Record, Archive Applicant Record, and Search / Filter Applicants.

Under **vacancy management**, the primary use cases are Add Job Vacancy and Manage Vacancy Records, the latter extended by Edit Vacancy and Activate / Deactivate Vacancy.

Under **recommendation and analytics**, the primary use cases are Generate Recommendation, View Recommendations, and View Analytics Dashboard. Generate Recommendation includes Select Applicant, Rank Active Vacancies, and Log Recommendation, and is extended by Notify No Active Vacancies when no active vacancy exists. View Recommendations is extended by Mark Viewed / Dismissed. View Analytics Dashboard includes View Job Seeker Panel and View Vacancy Panel and is extended by Filter by District. The recommendation model is pre-trained by the research team on verified historical placement records prior to deployment; model training is not a staff-facing function.
...
Figure 6 presents the ERD. The database consists of five core tables, corresponding one-to-one with the five data stores in the Level 1 Data Flow Diagram. The **users** table stores staff account credentials, full name, email, and active status. The **applicants** table stores individual applicant profiles, including demographic attributes (sex, civil status, barangay) and the four ML input fields: skills, education level, work experience, and preferred position; a single applicant record is created regardless of the entry method used (manual encoding or CSV/Excel batch upload), so the two entry methods are input options that populate this one table rather than separate entities. The **job_vacancies** table stores employer name, position title, and occupational category for each active vacancy; the occupational category is the label the recommendation engine scores against when ranking vacancies for a selected applicant. The **recommendations** table is the persistent log of recommendation outputs. Each row represents one ranked vacancy result from a single recommendation run, identified by the applicant and vacancy foreign keys, together with the suitability score, the timestamp of the run, and the staff member who triggered it. The status field records whether the recommendation output was viewed or dismissed. Multiple rows sharing the same applicant_id and recommended_at timestamp represent the full ranked list produced in one recommendation session. The **login_logs** table maintains a security audit trail of authentication events by user, timestamp, IP address, and outcome.
...
The process begins when PESO staff opens the Job Recommendation tab and selects an applicant from existing records or enters a new profile directly. Before proceeding, the system verifies that all four required fields are present: EDUC LEVEL, PREFERRED POSITION, SKILLS, and WORK EXPERIENCE. If any field is empty, the system prompts the staff to complete the missing information and the profile cannot proceed until all four fields are filled. All four fields are required because each contributes a distinct qualification signal — a profile with any missing field produces an incomplete text representation and an unreliable category score.
...
Each record's Job Position value is checked against a predefined mapping table. Records with unrecognizable job titles are excluded. Matched records are assigned to one of five occupational categories — Warehouse and Logistics, Production and Manufacturing, Sales/Service/Retail, Clerical and Administrative, and General Services and Security — because the raw Job Position column contains 88 unique titles, many appearing only once or twice, which is insufficient for reliable classifier training.
...
| Backend | Flask-WTF | 1.2 | Will provide CSRF token validation on all POST forms, protecting applicant and vacancy data against cross-site request forgery attacks |
...
| UI/UX Design | Figma | Web-based | Will be used to design high-fidelity mockups of all system screens prior to implementation, including the Job Recommendation tab, Analytical Dashboard, and applicant registration form |
...
1. **Perceived Usefulness (PU)** assesses whether the system helps staff do their job better. Sample items: *The ranked job recommendations help me identify suitable vacancies for applicants more efficiently than my current process*; *The Analytical Dashboard gives me a clearer picture of applicant and vacancy trends than my current process.*
...
**Respondents.** Twenty-seven (27) PESO CSJDM staff members who directly manage applicant data and referral operations, a number within the guideline range of 15 to 30 end-users, will be selected through purposive sampling to ensure respondents have first-hand experience with the workflows the system supports.
